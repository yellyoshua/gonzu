...
import { Post, Posts } from "@/app/interfaces/models";
import { copyObject } from "@/app/utils/transforms";
...
export class PostsRepository {
  ...
  #parsePost(post: PostOrPage): Post {
    const reading_time_text = readingTime(post, {
      minute: "1 minuto de lectura.",
      minutes: "% minutos de lectura.",
    });
    const tags_text = tags(post, { prefix: "Posted in: ", suffix: "." });

    const newPost: PostOrPage & {
  ...
  async getPosts(fields?: string, filter?: string): Promise<Posts> {
    const posts = await this.blogApi.posts.browse({
      limit: 20,
      filter,
      fields,
    });
